									...
									<div class="col-lg-12">
										<div class="donate-area">
											<ul class="nav nav-tabs" role="tablist">
												...
												<li class="active" role="presentation"><a >02. 上传证件图片</a></li>
											...
											<div class="tab-content">
												<div id="home" class="tab-pane active fade in" role="tabpanel">
												    <form action="${APP_PATH}/member/uploadImg.htm?fusm=10&mtype=${mtype}&id=${member.id}" enctype="multipart/form-data" method="post" id="certForm">
													<div class="donate-holder">
														...
														<div class="form-area">
															<div class="input-area">
																...
																<input type="file" class="certfile" data-name="${mtc.name}" data-cert-id="${mtc.certid}" data-mtype="${mtc.mtype}" name="imgfile-${mtc.certid}-${mtc.mtype}" placeholder="">
															</div>
															<#if mtc.iconpath??>
															<img id="img-${mtc.certid}-${mtc.mtype}" src="${APP_PATH}/cert/imgs/${mtc.iconpath}">
															<#else>
															<img id="img-${mtc.certid}-${mtc.mtype}" style="display:none;">
															</#if>
														</div>
														</#list>
														<div class="cs-holder">
															<input type="submit" onclick="window.location.href='${APP_PATH}/member/cert-1.htm?mtype=${mtype}';return false;" style="width: 48%;" value="< 基础信息">
															<input type="submit" onclick="nextFlow();return false;" style="width: 48%;" value="填写邮箱地址 >">
														</div>
													</div>
													</form>
												</div>
	...
	//	});
       ...
       $(".certfile").change(function(){
	        ...
	        if(file.size > 1024 * 1024 * 2) {
		        layer.msg(dataname+"图片大小不能超过 2MB!", {time: 1000, icon:5, shift:6, zIndex: ++layer.zIndex});
	            return false;
	        }
            ...
            if (file.type.startsWith("image/")) {
		        ...
		        $("#img-"+certid+"-"+mtype).attr("src", imgURL);
		        $("#img-"+certid+"-"+mtype).show();
            } else {
                layer.msg("请选择图形文件", {time: 1000, icon:5, shift:6, zIndex: ++layer.zIndex});
            }
       });
  });
	        ...
	        success : function(r) {
	            if ( r.success ) {
	                setTimeout(function(){
	                    ...
	                    window.location.href = "${APP_PATH}/member/cert-3.htm";
	                }, 500);
	            }
	        },
	        error : function(r) {
